...
function getFaceDataForShowChart() {
    ...
    showTodayList = store.state.faceData.list.map(currentData => {
        if (currentData.check_time > todayBegin && currentData.check_time < todayEnd) {
            let nowHour = new Date(parseInt(currentData.check_time)).getHours();
            return nowHour;
        }
        return -1;
    });
